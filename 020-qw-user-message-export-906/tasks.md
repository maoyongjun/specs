# 任务清单：906 私聊聊天记录 CSV 导出

**输入**：来自 `specs/020-qw-user-message-export-906/spec.md` 的功能规格  
**前置条件**：`spec.md`、`checklists/requirements.md`、`AGENTS.md`  
**测试**：当前阶段只验证 Spec Kit 文档存在且需求完整；后续实现阶段需要验证三账号范围、私聊过滤、群聊排除、`906` 前缀筛选和固定七列 CSV 输出，其中 `message_source` 保持原值、`isSelf` 按老师/学员转换，且 `union_id` / `timestamp` 可正确回填。
...
- [ ] T013 确认 `chat_name` 的字段来源和 `906` 前缀判定位置
