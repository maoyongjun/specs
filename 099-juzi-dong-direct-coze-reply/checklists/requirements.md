...
- [x] 改变调用顺序的业务语义已由用户确认：Dong 分支优先于私域和旧链路。

## 实施就绪度

- [x] 实现范围已限定，不扩散到无关模块。
- [x] 不新增数据库表、不新增对外 API、不修改 MQ 契约。
- [x] 已确认旧逻辑中必须保持不变的过滤、异常、日志、延迟和 fallback。
- [x] 每个关键需求至少有一条测试、编译或静态验证任务。
- [x] 常规单元测试计划避免真实访问 Redis、Coze、FC 或 Juzi；真实 Coze 验证通过显式开启的集成测试执行。

## 备注

- Dong 命中后失败也返回已处理，是为了避免旧链路产生重复或非预期回复。
