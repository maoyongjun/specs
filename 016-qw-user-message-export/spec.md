...
6. **Given** 某个学员已开口，**When** 输出明细，**Then** 结果中必须包含该学员的 `chat_name`，以及一条命中的 `message_id` 和 `payload`。
7. **Given** 某个学员未开口，**When** 输出明细，**Then** 结果中至少包含该学员的 `chat_name`。
...
运营或数据整理人员还需要一个“全量学员模式”，例如通过 `--mode open-rate-all` 运行，用于查看 5 月 8 日到统计执行当天这段时间内所有新学员的开口情况。这里的新学员定义为：在 5 月 8 日之前没有任何聊天记录的学员。该模式不再要求学员必须先命中建联文案，也就是说，只要在统计窗口内有符合当前筛选口径的学员消息，就纳入统计；但在判断开口时，仍然要排除 `我已经添加了你，现在我们可以开始聊天了。` 和 `我通过了你的联系人验证请求，现在我们可以开始聊天了` 这两句招呼语本身。
...
- 结果输出需要读取 `message_id`、`payload`、`chat_name`、`external_user_id`、`user_id` 等字段，以支持后续开口率统计和名单输出；若 `chat_name` 为空，可回退读取同表的 `contact_name`。
...
- 已开口名单至少输出 `chat_name`、`message_id`、`payload`。
- 未开口名单至少输出 `chat_name`。
...
- `chat_name` 应沿用现有联系人信息口径获取，若存在多种来源，后续实现需在编码前统一；必要时可回退到同表的 `contact_name`。
...
- **FR-031**：新增统计功能 MUST 输出已开口学员的 `chat_name`、一条样本 `message_id` 和 `payload`。
- **FR-032**：新增统计功能 MUST 输出未开口学员的 `chat_name`。
...
- **SC-019**：规格明确已开口名单输出 `chat_name`、`message_id`、`payload`，未开口名单输出 `chat_name`。
...
- `chat_name` 可以通过现有联系人信息口径获取，若存在多种来源，后续实现需统一采用同一口径。
