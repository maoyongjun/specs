...
- [x] 不存在未处理的调用后赋值风险。
- [x] Redis key、TTL 和返回覆盖已有下游参数断言方案。
- [x] 本次新增 Redis 短期状态已由用户确认。

## 实施就绪度

- [x] 实现范围已限定，不扩散到无关模块。
- [x] 不新增数据库表、不新增对外 API、不修改 MQ/Redis 既有契约；仅新增规格明确的 Redis key。
