...
- [x] 若修复会改变调用顺序、接口契约、远程调用或业务语义，已记录；自然日口径已完成用户确认。
...
- 当前自然日口径：
  - “当天提交超过两次”按自然日判断，与课程/作业天数 `submitDay/currentDay` 无关。
  - 自然日使用 `Asia/Shanghai` 的 `yyyyMMdd`。
  - `WX_006` 对同一 `external_user_id + naturalDate` 只提醒一次，避免第 3 次之后每次重复提醒。
