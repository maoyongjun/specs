# 规格质量检查清单：external-info-select 返回 speaker_name
...
- [x] 明确用户目标、成功标准和非目标。
- [x] 明确新增、修改和禁止改变的行为。
- [x] 明确日志、时间、缓存、fallback、兼容性和异常处理要求。
...
- `speaker_name` 查不到时省略字段，不返回空字符串。
- 本地缓存只缓存成功查到的非空 name，TTL 为 6 小时。
