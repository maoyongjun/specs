# 规格质量检查清单：common_warn_sender 支持 FX_002 私域变量与“请勿打扰”打标

**用途**：验证需求完整性、参数完整性和实施就绪度  
**创建日期**：`2026-06-16`  
**功能**：[spec.md](../spec.md)

## 内容质量

- [x] 明确目标项目、模块、入口和核心实现位置。
- [x] 明确用户目标、成功标准和非目标。
- [x] 明确新增、修改和禁止改变的行为。
- [x] 明确日志、幂等、fallback、兼容性和异常处理要求。
- [x] 明确后续实现必须增加测试或静态验证记录。

## 需求完整性

- [x] 无 `[NEEDS CLARIFICATION]` 或未替换占位内容残留。
- [x] 需求可测试且无明显歧义。
- [x] 成功标准可衡量。
- [x] 验收场景覆盖正常路径、边界路径和不回归路径。
- [x] 边界情况已识别，并明确跳过、兜底、抛错或记录日志的策略。

## 参数完整性门禁

- [x] 已列出关键参数来源和赋值时机。
- [x] 已列出下游读取字段清单。
- [x] 没有未解释的 `new XxxDto()`、空 JSON、空 Map 或占位参数。
- [x] 下游读取字段在调用前已赋值，或在当前层现算现用。
- [x] 不存在未处理的调用后赋值风险。
- [x] 外部接口、HTTP 和数据库查询的关键参数已有下游参数断言方案。
- [x] 已记录并完成用户确认：标签名为“请勿打扰”，tagId 按 `source + name` 查询 `drh_qw_tag`。
- [x] 已记录用户补充：`FX_002` 飞书接收人固定为 `ed27a7bb`。

## 实施就绪度

- [x] 实现范围已限定，不扩散到无关模块。
- [x] 不新增数据库表；新增内部查询接口已在规格明确。
- [x] 已确认旧逻辑中必须保持不变的解析、发送、跳转链接、Redis 频控和失败 fallback。
- [x] 每个关键需求至少有一条测试、编译或静态验证任务。
- [x] 单元测试计划避免真实访问 Redis、OTS、Center、RocketMQ、FC 或外部 HTTP。
- [x] 用户纠正“请勿打扰”标签名后，已同步更新 `spec.md`、`tasks.md` 和 `AGENTS.md`。

## 备注

- `FX_002` 打标为 best-effort；发送成功和打标成功分别记录。
- `markAsync` 返回成功只表示异步任务提交，最终以现有任务表和任务日志为准。
