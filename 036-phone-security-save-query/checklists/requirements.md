...
- [x] 明确用户目标（保存同步写入安全字段、查询使用 MD5、展示使用掩码、前端兼容、历史回填）、成功标准（9 条 SC）和非目标（在线代码不改造 `app_phone`）。
...
- [x] 需求可测试且无明显歧义。
- [x] 成功标准可衡量（9 条 SC）。
- [x] 验收场景覆盖保存（正常 + 明文兼容 + 空值）、查询（正常 + 空值 + 不匹配）、展示（正常 + 历史 NULL）、前端兼容（明文 + 密文）、单元测试和历史回填六个路径。
...
- [x] 每个关键需求（FR-001 到 FR-014）都有对应的实现任务和测试任务。
