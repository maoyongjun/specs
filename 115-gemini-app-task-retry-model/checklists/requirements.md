# 规格质量检查清单：Gemini AppTask 重试模型切换
...
- [x] 已覆盖 `Content-Length` 超限、未知、不可信和恰好 5MiB 的边界。
...
- [x] 若修复会改变调用顺序、接口契约、远程调用或业务语义，已记录并完成用户需求口径确认；无额外设计选择。
- [x] AppTask mp3 裁剪只影响当前层现算的 `audioBase64`，不依赖下游补齐字段。
...
- [x] mp3 大小裁剪测试使用内存 InputStream，不访问真实 URL。

## 备注

- Phase 1 / Phase 2 门禁已完成。
- D003 mp3 大文件裁剪已实现并验证通过。
