# 规格质量检查清单：0428 营期作业状态回退与历史点评 union_id 备份
...
- [x] 目标营期固定为 4 个 0428 进阶班。
- [x] 目标课程固定为 `胡琴说（下）`。
- [x] workPicId 目标数为 `1972`。
- [x] 历史点评目标数为 `855`。
- [x] `UPDATE drh_works_pic` 只设置 `status=0`。
- [x] `UPDATE drh_history_pic` 只设置 `union_id=target_union_id`。
- [x] 写入范围使用临时表固化的 `work_pic_id` / `history_id`。
- [x] 仓库内 SQL 默认以 `ROLLBACK` 收尾。
- [x] 临时 `COMMIT` 副本已执行并通过提交后验证。

## 提交后结果

- [x] `drh_works_pic`：4 个营期共 `1972` 条目标作业全部 `status=0`。
- [x] `drh_history_pic`：原始 `union_id` 命中为 `0`。
- [x] `drh_history_pic`：`_bak` 命中为 `855`。
- [x] 提交后验证结果已导出到 `verify-drh-0428-workpic-history-bak-after.csv`。

## 备注

- 首次演练暴露了 DDL 隐式提交风险；已在脚本和文档中明确规避方式。
- 回滚必须基于本目录的两个预览 CSV 快照重新构造临时表，并重新走 `database-sql-skill` 写入门禁。
