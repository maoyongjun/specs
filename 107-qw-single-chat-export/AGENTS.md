...
本目录记录"单账号 5 月 23 日至 6 月 23 日私聊导出"的 Spec Kit 文档。本规格为文档与实现一体：先落文档，再在 `qw-user-message-export` 新增导出模式并补测试验证。

## 作用范围

- 规格文档：`C:\workspace\ju-chat\specs\107-qw-single-chat-export`
- 目标项目：`C:\workspace\ju-chat\qw-user-message-export`
- 相关模块：Java 8 Maven CLI，包名 `com.juchat.qwexport`

## 当前目标

- 新增专用 CLI 模式 `--mode single-chat-export`，不覆盖现有 `export`、`jun3-chat-export`、`may15-chat-export`、`open-rate`、`open-rate-all`、`activity-rate`、`interaction-rate`、`piano-daily-open-rate` 等模式。
- 通过 OTS 表 `juzi_private_message` 和索引 `juzi_private_message_index` 导出固定 `15110230427` 的**私聊**聊天记录。
- 口径与既有 `export`/`jun3-chat-export`/`may15-chat-export` 一致：**只导出私聊**，排除群聊（`is_group=true`），复用私聊查询 `findExportMessages`/`buildExportRequest`。
- 固定账号（企业微信员工 `user_id`）：`15110230427`。
- 固定时间窗口为 `2026-05-23 00:00:00 Asia/Shanghai` 到 `2026-06-23 23:59:59.999 Asia/Shanghai`，区间两端包含，且为固定窗口（不依赖运行时刻）。
...
- 单个账号查询、单条消息解析或单行写入失败时，应记录错误并继续处理其他消息。
- 测试不能只断言最终文件存在；必须断言下游 OTS 查询参数（固定账号、固定时间窗口、`user_id` 字段、私聊过滤即排除群聊、时间闭区间、timestamp 升序）和 CSV 字段内容。

## 强制门禁

编码前必须完成并记录以下检查：

- 关键参数来源：固定账号 `15110230427`、固定开始时间、固定结束时间、OTS 字段名和输出目录。
- 赋值时机：账号和时间窗口必须在调用 `OtsMessageRepository` 前确定，均为编译期常量推导，无运行期占位。
