...
本目录是新需求 Spec Kit。分析 `/liveAuth/ad/applet/v4` 接口在处理明文手机号时产生的 AES 解密 ERROR 日志问题。

## 作用范围

- 规格文档：`C:\workspace\ju-chat\specs\089-liveauth-v4-aes-decrypt-error`
- 目标项目：`C:\workspace\drh`
- 相关模块：`drh-common`（DataSecurityUtil、DataSecurityInvoke）、`drh-endpoint`（LiveAuthController、LiveAuthServiceImpl）

## 当前目标

- 定位 `/liveAuth/ad/applet/v4` 接口报错 `DataSecurityUtil: AES aesDecrypt failed:Input length must be multiple of 16 when decrypting with padded cipher` 的根因。
- 分析调用链路中 `normalizePhoneInput` 对明文手机号"盲解密"的设计缺陷。
- 提出修复方案：在尝试 AES 解密前增加密文预判，并将预期异常的日志级别从 ERROR 降为 DEBUG。
...
- 入口：`LiveAuthController.adAuth()` -> `LiveAuthServiceImpl.saveAppletUser()`（`drh-endpoint`）
- 核心实现：`DataSecurityInvoke.normalizePhoneInput()` -> `DataSecurityUtil.aesDecrypt()`（`drh-common`）
- 测试类：`DataSecurityUtilTest`（`drh-common/src/test`）
