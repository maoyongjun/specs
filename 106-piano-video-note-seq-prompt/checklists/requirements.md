...
- [x] D017 明确真实异步 FC/Redis 音序回归结果、V5 实际提取音序与固定 D5 模板偏差、萱草花开头指纹优先规则，以及 Redis 凭据不落库/不入文档的安全边界。
...
- [x] D017 已补充萱草花开头指纹的触发位置、证据输出、与 D2 结尾 E / D1 纠偏 / 低分人工介入规则的优先级边界。
...
- [x] D017 验证覆盖 D2 当前进度下 5 个 demo 视频的真实异步音序工程矩阵，并对 V5 修正后补完整 `PianoHomeWorkVideoV2Task` 主链路抽验；未声称重跑 5 个视频完整 Gemini 主链路。
...
- D017 已实施：真实异步 FC/Redis 音序工程矩阵按 D2 当前进度 5/5 PASS；V5 初次失败后新增萱草花开头指纹并复验通过，完整主链路抽验输出 D5/提前提交。
