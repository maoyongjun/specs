...
- D012/D013 模板优先门禁：只有 D1/D2/D3 模板达到高置信规则时才覆盖课程分类；覆盖范围限制在分类字段，不能抹掉 Gemini 的诊断字段；低置信不覆盖。若 Gemini 原分类证据与工程覆盖冲突，最终 evidence 必须替换为工程覆盖说明。
...
- `fc/Gemini-Api/src/main/java/com/drh/gemini/api/PianoNoteSequenceTemplateMatcher.java`：D1/D2/D3 音序模板匹配器，输出模板分数、高置信工程判定，并处理 D2 结尾连续 E 音级优先规则。
