...
- [x] T010 `picUrl` 为空时直接发送失败回调并跳过后续分析流程
- [x] T011 异步失败分支改为 `return null` 并打印 `error` 日志

## Phase 3：验证

- [ ] T012 复查代码确保两类告警文案都包含 `unionId`、`songName`、`nickName`、`picId`、`classId`
- [ ] T013 复查飞书调用签名与参考实现一致
- [ ] T014 复查 `retry(...)` 60 秒常规延迟失败时会发送告警并尝试 3300 秒兜底重试，且 `retry(...)` 正常提交成功时不会发送告警
- [ ] T015 复查 `picUrl` 为空时会直接发送失败回调且不进入后续分析流程
- [ ] T016 复查异步失败分支会打印 `error` 日志并返回 `null`
- [ ] T017 记录验证结果和剩余风险
- [ ] T018 复查常规延迟保持 60 秒，兜底延迟保持 3300 秒（55 分钟）
