...
- 当 `picUrl` 为空时，直接发送失败回调并跳过后续分析流程。
- 由于异步入口失败应 `return null` 并打印 `error` 日志，而不是继续抛出异常。
...
- 只修改 `AppTask.java` 中 `picUrl` 为空时的失败回调分支和异步失败出口相关逻辑。
- 飞书告警失败不得覆盖原始任务失败异常。
- 不改变现有回调、重试、日志和任务处理主流程语义。
- `retry(...)` 正常提交成功时不新增飞书告警，也不触发 3300 秒兜底延迟重试。
- 失败分支不得继续向上抛出异常，应通过 `return null` 结束。
