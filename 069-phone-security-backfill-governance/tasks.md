...
- [x] T018 文档记录硬顺序：字段 DDL -> 索引 DDL -> 加密字段回填 -> 抽样核对 -> dry-run -> 明文掩码覆盖。
- [x] T019 文档记录明文掩码覆盖后不得重跑加密字段回填。
- [x] T020 明文掩码覆盖 SQL 只在 `mask/aes` 非空且 `source != mask` 时更新。
- [x] T021 明文掩码覆盖不调用 FC，不改 `md5/aes`。

## Phase 5：验证

- [x] T022 新增服务层单测，覆盖 SQL 门禁、dry-run、互斥和失败目标继续处理。
- [x] T023 新增控制器单测，覆盖受理、拒绝和状态查询。
- [x] T024 执行 `juzi-service` 相关单测。
- [ ] T025 在测试库执行最终 DDL 和 dry-run。
- [ ] T026 在生产上线窗口前抽样核对 `mask/md5/aes` 准确性。
