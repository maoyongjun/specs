...
用户打开 Windows exe 后，可以在 JavaFX 界面中看到固定 1-20 个账号槽位、每个账号的邮箱/过期时间/账号目录，并点击账号启动 Cursor 或 Codex。

**独立测试**：在存在或不存在账号文件的机器上启动应用，验证界面显示 1-20 个槽位，点击某个槽位会准备对应目录并以该槽位的 `CODEX_HOME` 启动 Cursor 或 Codex。

**验收场景**：

1. **Given** 应用启动，**When** 主界面加载完成，**Then** 左侧固定显示账号 1-20。
...
**独立测试**：在临时用户目录中执行准备全部账号，验证 20 个账号目录、共享目录、默认配置和共享链接均生成。
...
**独立测试**：执行导出后检查 zip，验证存在 `manifest.properties`、`accounts/account1..20`、`shared` 和 `legacy` 目录，且账号目录中的共享链接不会重复跟随导出。

**验收场景**：

1. **Given** 用户点击导出全部账号，**When** 选择 zip 保存路径，**Then** 应用生成明文 zip。
2. **Given** 导出成功，**When** 打开 zip manifest，**Then** 包含固定字段：`schemaVersion=1`、`appVersion=1.0.0`、`createdAt`、`maxAccounts=20`、`exportSecurity=plainZip`、`includesShared=true`。
...
1. **Given** 目标电脑已有账号数据，**When** 用户恢复 zip，**Then** 现有 `.codex-account1..20`、`.codex-shared` 和相关 `.codex` 文件先被备份。
