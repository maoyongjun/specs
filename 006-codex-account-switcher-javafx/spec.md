...
用户打开 Windows exe 后，可以在 JavaFX 界面中看到固定 1-15 个账号槽位、每个账号的邮箱/过期时间/账号目录，并点击账号启动 Cursor 或 Codex。

**独立测试**：在存在或不存在账号文件的机器上启动应用，验证界面显示 1-15 个槽位，点击某个槽位会准备对应目录并以该槽位的 `CODEX_HOME` 启动 Cursor 或 Codex。

**验收场景**：

1. **Given** 应用启动，**When** 主界面加载完成，**Then** 左侧固定显示账号 1-15。
...
**独立测试**：在临时用户目录中执行准备全部账号，验证 15 个账号目录、共享目录、默认配置和共享链接均生成。
...
2. **Given** 导出成功，**When** 打开 zip manifest，**Then** 包含固定字段：`schemaVersion=1`、`appVersion=1.0.0`、`createdAt`、`maxAccounts=15`、`exportSecurity=plainZip`、`includesShared=true`。
