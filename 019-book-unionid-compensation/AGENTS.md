...
- `unionId` 相关查询要明确区分“排空字符串”与“排 `NULL` + 空字符串”，需要时保持 `isNotNull(...).ne(..., "")`。
- 任何 `externalUserId` 为空的分支，都要先确认是否还存在 `AppletUserDo` 兜底；不要把兜底前置条件写成直接 `continue`。
- 只有在消息内容、`empId` 和回写条件都满足后，才回写 `drh_book_question_record.union_id`。
- 修改补偿流程时，要同步检查分页条件、跳过原因、日志和单条异常是否仍然继续处理后续记录。
