...
- 参数来源：`observed`（pitchClasses）来自 `PianoNoteSequenceFeatureExtractor.buildPitchClasses`，在 `match()` 调用前已赋值；`baseBestScore` 在 `match()` 内、调用 `bestTransposedRescue` 前由原始最佳分算出；`transposed`（含 `contiguousPhraseSimilarity`）由 `scoreAgainstPitchClasses` 计算并在读取前已赋值。
...
- 下游读取：`PianoHomeWorkVideoV2Task` 依据 `templateMatch.highConfidence` 与 `bestScore.score` 决定走「人工复核短路」还是覆盖；本次只让噪声序列 highConfidence 回到 false，下游既有分支自然命中低分人工复核。
