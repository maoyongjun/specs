...
- [x] 不存在未处理的调用后赋值风险。
- [x] FC 关键参数已有单元测试断言方案。
- [x] 新增 API 属于用户明确要求，已在规格中记录接口契约。

## 实施就绪度

- [x] 实现范围已限定，不扩散到无关模块。
- [x] 不新增数据库表、不修改 MQ/Redis/配置契约。
- [x] 已确认旧逻辑中必须保持不变的过滤、异常、日志和 fallback。
- [x] 每个关键需求至少有一条测试、编译或静态验证任务。
- [x] 单元测试计划避免真实访问 Redis、OTS、Center、RocketMQ、FC 或外部 HTTP。
- [x] 补充需求或纠正需求时，同步更新 `spec.md`、`tasks.md` 和 `AGENTS.md`。

## 备注

- 本接口返回明文手机号，必须依赖现有登录态保护；后续如需按钮权限或审计日志，应追加新规格。
