...
- [x] 明确门禁适用于所有命中 SOP 点评的 SKU，不限制为 `skuId=4`。
- [x] 明确 `skuId=5` 等非 4 SKU 也必须校验同一套门禁。
- [x] 明确 `aiStatus == 1` 才视为 AI 开启。
- [x] 明确 `aiAutoReview == 1` 才视为作业点评开启。
- [x] 明确两个开关都开启是调用 `sop-reply` 的共同前置条件。
- [x] 明确关闭、空配置、异常配置均不调用 `sop-reply`。
...
- [x] 需求可测试且无明显歧义。
- [x] 成功标准可衡量。
- [x] 验收场景覆盖 `skuId=4` 和 `skuId=5`。
- [x] 验收场景覆盖开关全开、AI 关闭、作业点评关闭和配置异常。
...
- 当前阶段仅完成文档，未进行代码实现。
- 开启值沿用 `AppTask.java` 现有口径：`1` 表示开启，其他值或空值均视为未开启。
- 群 ID 匹配沿用 `AppTask.java#isGroupOpen` 的精确包含口径：`chatList.contains(roomWecomChatId)`。
