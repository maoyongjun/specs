...
- [x] 已确认旧逻辑中必须保持不变的过滤、异常、日志和输出。
- [x] 每个关键需求至少有一条测试、编译或静态验证任务。
- [x] 单元测试计划避免真实访问 OTS。
- [x] 补充需求时已同步更新 `spec.md`、`tasks.md` 和 `AGENTS.md`。

## 备注

- 真实跑数仍依赖本机配置 OTS 环境变量：`endpoint`、`accessKey`、`accessSecret`、`instance`。
