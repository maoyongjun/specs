...
- [x] 不存在未处理的调用后赋值风险。
- [x] 数据库查询关键参数已有静态验证方案。
- [x] 本次修复不改变调用顺序、接口契约、远程调用或额外业务语义。
...
- 本次变更只追加两个数据库查询的 `createTime` 下限条件。
- 若后续业务要求“14 天内”同时排除未来时间，应追加 `createTime <= now` 并同步更新规格。
