-- 0310 营期《胡琴说（下）》作业状态回退、历史点评 union_id 备份、五维分数 union_id 备份
...
SET @expected_work_pic_count := 1623;
SET @expected_history_count := 114;
SET @expected_song_score_count := 579;

DROP TEMPORARY TABLE IF EXISTS tmp_drh_0310_workpic_targets;
CREATE TEMPORARY TABLE tmp_drh_0310_workpic_targets AS
SELECT
...
ALTER TABLE tmp_drh_0310_workpic_targets ADD PRIMARY KEY (work_pic_id);

SELECT COUNT(*) INTO @target_work_pic_count
FROM tmp_drh_0310_workpic_targets;

DROP TEMPORARY TABLE IF EXISTS tmp_drh_0310_history_targets;
CREATE TEMPORARY TABLE tmp_drh_0310_history_targets AS
SELECT
  hp.id AS history_id,
  hp.pic_id,
  wp.id AS work_pic_id,
...
JOIN tmp_drh_0310_workpic_targets t ON wp.id = t.work_pic_id
WHERE wp.is_del = 0
AND hp.union_id NOT LIKE '%\\_bak' ESCAPE '\\';

ALTER TABLE tmp_drh_0310_history_targets ADD PRIMARY KEY (history_id);

SELECT COUNT(*) INTO @target_history_count
FROM tmp_drh_0310_history_targets;

DROP TEMPORARY TABLE IF EXISTS tmp_drh_0310_song_score_targets;
CREATE TEMPORARY TABLE tmp_drh_0310_song_score_targets AS
SELECT
...
ALTER TABLE tmp_drh_0310_song_score_targets ADD PRIMARY KEY (song_score_id);

SELECT COUNT(*) INTO @target_song_score_count
FROM tmp_drh_0310_song_score_targets;
...
SELECT
  'before_song_score' AS phase,
  camp_id,
  camp_name,
  live_id,
  COUNT(*) AS song_score_count,
  COUNT(DISTINCT pic_id) AS song_score_pic_count,
  COUNT(DISTINCT original_union_id) AS song_score_union_count
FROM tmp_drh_0310_song_score_targets
GROUP BY camp_id, camp_name, live_id
ORDER BY camp_id, live_id;
...
JOIN tmp_drh_0310_history_targets t ON hp.id = t.history_id
...
JOIN tmp_drh_0310_song_score_targets t ON ss.id = t.song_score_id
JOIN drh_works_pic wp ON wp.id = t.work_pic_id
SET ss.union_id = t.target_union_id
WHERE @target_work_pic_count = @expected_work_pic_count
AND @target_song_score_count = @expected_song_score_count
AND wp.status = 0
AND wp.is_del = 0
AND ss.pic_id = t.pic_id
AND ss.class_id = t.live_id
AND ss.union_id = t.original_union_id;

SET @song_score_changed_rows := ROW_COUNT();
...
FROM tmp_drh_0310_history_targets t
LEFT JOIN drh_history_pic hp ON hp.id = t.history_id;

SELECT
  SUM(CASE WHEN ss.union_id = t.target_union_id THEN 1 ELSE 0 END),
  SUM(CASE WHEN ss.union_id <> t.target_union_id OR ss.union_id IS NULL THEN 1 ELSE 0 END)
INTO @song_score_updated_count, @song_score_not_updated_count
FROM tmp_drh_0310_song_score_targets t
LEFT JOIN drh_song_score ss ON ss.id = t.song_score_id;
...
SELECT
  ...
  @target_song_score_count AS target_song_score_count,
  @song_score_changed_rows AS song_score_changed_rows,
  @song_score_updated_count AS song_score_updated_count,
  @song_score_not_updated_count AS song_score_not_updated_count;
